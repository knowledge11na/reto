...
export default function MyPage() {
  ...
  const handleGotoGacha = (e) => {
    if (!canDrawGacha) {
      e.preventDefault();
      alert(
        'ガチャには 500 ベリー必要です。クイズや投稿でベリーを集めてください！'
      );
    }
  };
    ...
    <div className="min-h-screen bg-sky-50 flex flex-col items-center text-sky-900">
      ...
      <main className="w-full max-w-md px-4 pb-10 mt-4 space-y-4">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="space-y-1">
              ...
              <div className="mt-1">
                <Link
                  href="/titles"
                  className="inline-block px-2 py-1 rounded-full bg-white border border-sky-400 text-[11px] font-bold text-sky-700"
                >
                  称号一覧・変更 ＞
                </Link>
              </div>
        ...
        <section className="bg-sky-100 border-2 border-sky-500 rounded-3xl p-4 shadow-sm">
          ...
          <div className="text-sm space-y-1">
            <p>シーズン最高：{seasonBestCorrect} 問正解</p>
            <p>歴代最高：{allTimeBestCorrect} 問正解</p>
          </div>
          <p className="mt-2 text-[11px] text-sky-700">
