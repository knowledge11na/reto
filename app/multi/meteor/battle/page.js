...
export default function MultiMeteorBattlePage() {
  ...
  // ★後攻の「最初から30秒削れてる」をバーで見せたいので、
  //   2人とも「最大値は先攻(=7:00)」で統一して描画する
  ...
  const HpBar = ({ label, ms, maxMs, tone = 'me' }) => {
    ...
    return (
      <div className="w-full">
        ...
        {/* ★後攻の「最初から短い」を説明したいならこれを出せる */}
        {/* <div className="mt-1 text-[10px] text-slate-400">最大は7:00基準</div> */}
      </div>
    );
  };
        ...
        {state && (
          <>
            ...
            <div className="mt-3 text-[11px] sm:text-xs text-slate-300">
              HPバーは「最大7:00基準」で統一表示（後攻は最初から短く見える）。隕石にも残り時間バーを表示。
            </div>
          </>
        )}
      </div>
