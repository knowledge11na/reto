...
export default function AdminQuotesPage() {
  ...
  // 挿入（間に刺す）
  const [insertAfterId, setInsertAfterId] = useState(null); // null=先頭に挿入
...
const [bulkText, setBulkText] = useState('');
  ...
  const handleAddQuote = async () => {
    ...
    const name = String(charInput || '').trim();
    const text = String(quoteText || '').trim();

    if (!ep) return setMsg('話数が不正です。');
    if (!name) return setMsg('キャラ名を入力してください。');
  ...
  const openInsert = (afterIdOrNull) => {
    ...
    setInsertChar(charInput || ''); // ついでに現在入力を引き継ぐ
    setInsertText('');
  };
  ...
  const handleInsert = async () => {
    setMsg('');
    const ep = clampEpisode(episode);
    const name = String(insertChar || '').trim();
    ...
    try {
      const res = await fetch('/api/admin/quotes', {
        ...
        body: JSON.stringify({
          ...
          character_name: name,
          quote_text: text,
          insert_after_id: insertAfterId, // ★ここ
        }),
      });
      const data = await res.json().catch(() => ({}));
  ...
  const startEdit = (row) => {
    ...
    setEditChar(String(row?.character_name || ''));
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch('/api/admin/quotes', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, character_name: name, quote_text: text }),
      });
      const data = await res.json().catch(() => ({}));
    ...
    <main className="min-h-screen bg-sky-50 text-slate-900 p-4 md:p-6">
      <div className="max-w-5xl mx-auto space-y-5">
        <header className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-700 mt-1">
              話数・キャラ・セリフを保存し、話数/キャラ/セリフで検索できます。各行は編集可能で、行と行の間に挿入もできます。
            </p>
          </div>
          <Link
            href="/admin"
        ...
        <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-5 space-y-3">
          <h2 className="font-extrabold text-slate-900">追加</h2>
            ...
            <label className="space-y-1 md:col-span-2">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <input
                  ...
                  className="flex-1 min-w-[240px] rounded-xl border border-slate-300 px-3 py-2 bg-white text-slate-900"
                  placeholder="例：ルフィ（入力すると候補が出ます）"
                />
                ...
                <button
                  type="button"
                  onClick={() => {
                    // キャラ追加（CSVに追記するAPIがPOST対応している前提）
                    fetch('/api/admin/quotes/chars', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ name: String(charInput || '').trim() }),
                    })
              ...
              <CharSuggest value={charInput} onPick={(name) => setCharInput(name)} />
            </label>
          </div>
            ...
            <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-3 space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                <label className="space-y-1 md:col-span-1">
                  <div className="text-xs font-bold text-slate-700">キャラ</div>
                  <input
                    value={insertChar}
                    onChange={(e) => setInsertChar(e.target.value)}
                    className="w-full rounded-xl border border-slate-300 px-3 py-2 bg-white text-slate-900"
                    placeholder="例：シャンクス"
                  />
                </label>
        ...
        <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-extrabold">この話数のセリフ（保存順）</h2>
            <div className="text-xs text-slate-600">
              {episodeLoading ? '読み込み中…' : `${episodeRows?.length ?? 0} 件`}
            </div>
          </div>
            ...
            <table className="min-w-[760px] w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-slate-600">
                  ...
                  <th className="py-2 pr-3 w-[220px]">キャラ</th>
                      ...
                      <td className="py-2 pr-3 font-bold text-slate-900">
                        {isEditing ? (
                          <input
                            ...
                            className="w-full rounded-lg border border-slate-300 px-2 py-1 bg-white text-slate-900"
                          />
                        ) : (
                          r.character_name
                        )}
              ...
              <div key={g.episode} className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-[760px] w-full text-sm">
                    <thead>
                      <tr className="text-left text-xs text-slate-600">
                        <th className="py-2 pr-3 w-[220px] px-3">キャラ</th>
                        <th className="py-2 pr-3">セリフ</th>
                        <th className="py-2 pr-3 w-[220px]">操作</th>
                      </tr>
                    </thead>
